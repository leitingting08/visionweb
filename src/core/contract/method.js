...
export default class Method {
    constructor(contract, abi) {
        this.visionWeb = contract.visionWeb;
    ...
    async _send(
        ...
        args,
        options = {},
        privateKey = this.visionWeb.defaultPrivateKey,
        callback = false
    ) {
        ...
        try {
            const address = privateKey
                ? this.visionWeb.address.fromPrivateKey(privateKey)
                : this.visionWeb.defaultAddress.base58;
            const transaction = await this.visionWeb.transactionBuilder.triggerSmartContract(
                this.contract.address,
                this.functionSelector,
                options,
                parameters,
...
>>>>>>> upstream/master
            ...
            const signedTransaction = await this.visionWeb.vs.sign(
            ...
            const broadcast = await this.visionWeb.vs.sendRawTransaction(
                signedTransaction
            );
            ...
            if (broadcast.code) {
                ...
                if (broadcast.message)
                    err.message = this.visionWeb.toUtf8(broadcast.message);
                return callback(err);
            }
            ...
            const checkResult = async (index = 0) => {
                ...
                const output = await this.visionWeb.vs.getTransactionInfo(
                ...
                if (output.result && output.result === "FAILED") {
                    return callback({
                        error: this.visionWeb.toUtf8(output.resMessage),
                        transaction: signedTransaction,
                        output,
                    });
                }
    ...
    async _watch(options = {}, callback = false) {
        ...
        const getEvents = async () => {
            try {
                ...
                const events = await this.visionWeb.event.getEventsByContractAddress(
                    this.contract.address,
                    params
                );
